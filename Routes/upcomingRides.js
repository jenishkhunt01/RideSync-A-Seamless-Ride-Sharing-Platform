...
router.post("/finish/:rideId", ensureAuthenticated, async (req, res) => {
  try {
    ...
    let riderToUpdate = await userCollection.find({
      username: riderUsername,
    });
    let driverToUpdate = await userCollection.find({
      username: ride.driverId,
    });
    const rideCountUpdate = (riderToUpdate["number_of_rides_taken"] || 0) + 1;
    const driveCountUpdate = (driverToUpdate["number_of_rides_given"] || 0) + 1;

    await userCollection.updateOne(
      { username: riderToUpdate.username },
      { $set: { number_of_rides_taken: rideCountUpdate } }
    );

    await userCollection.updateOne(
      { username: driverToUpdate.username },
      { $set: { number_of_rides_given: driveCountUpdate } }
    );
